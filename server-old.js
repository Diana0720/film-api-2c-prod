...
app.delete('/movies/:id', [authenticateToken, authorizeRole('admin')], (req, res) => {
  db.run('DELETE FROM movies WHERE id=?', [req.params.id], function (err) {
    if (err) return res.status(500).json({ error: err.message });
      ...
      console.log(`Film dihapus oleh user: ${req.user.username}`);
...
app.put('/directors/:id', authenticateToken, (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      ...
      console.log(`Sutradara diperbarui oleh user: ${req.user.username}`);
